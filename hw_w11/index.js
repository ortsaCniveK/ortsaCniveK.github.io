...
//establish a list array (BIG)
//can probably find some way of doing this cleaner with fs read and write file.
...
//create word object using randomWord
let word = new Word(randomWord);

//establish number of guesses to start (will run 6 times as normal hangman)
let guessesLeft = 6;
let guesses = [];
...
const playGame = function(){
	...
	if (guessesLeft === 0){
		//set all the letters to true for display
		word.toggle();
		console.log('YOU LOSE! GOOD DAY SIR!\nThe word was: ' + word);
		return;
	}

	//output the current guesses
	if (guesses.length !== 0){
	...
	inquirer.prompt([
			...
			//if the user has already guessed that letter,
			if (guesses.includes(answer.guess)){
				//let them know, but do nothing.
				console.log('\nYou have already guessed that letter!\n');
			} else {
				//if the user has not guessed the letter
				//append to the guesses array
				guesses.push(answer.guess);

				//check the user's guess
				//if it does not return true, decrement the guessesleft
				if (!word.guess(answer.guess)){
					console.log('\nNope!');
					guessesLeft--;
				}
				...
				if (word.status()){
					//return out of the program by breaking out of the prompt loop
					console.log('\nCorrect!  The word was ' + word + '. Run the game to play again :)');
					guessesLeft = 0;
					return;
				}
			}

			//recursive call and continuing game
			playGame();
		})
}
